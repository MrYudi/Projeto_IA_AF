...
for directory in os.listdir(IMG_SAVE_PATH):
    ...
    for item in os.listdir(path):
        ...
        img = cv2.resize(img, (60, 60)) #pre-procesamento
...
# Os dados precisa esta num formato que a rede entenda o resultado
...
print("Arquitetura")
model = Sequential([
    SqueezeNet(input_shape=(60, 60, 3), include_top=False),
    Dropout(DROPOUT),
...
if not os.path.isdir(GRAFICO_PATH): 
    os.mkdir(GRAFICO_PATH) # Cria a pasta Dataset, caso nao exista
...
plt.savefig(GRAFICO_PATH+'/acuracia_'+timestampStr+'.png')
plt.clf()
...
plt.xlabel('Epoca')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig(GRAFICO_PATH+"/perda_"+timestampStr+".png")
plt.clf()

# Salva o modelo
model.save("rede-model.h5")